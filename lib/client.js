...
try {
    ...
    console.log("🌍 Connected to the THARINDU DB");
} catch {
...
let cc = Config.sessionName.replace(/THARINDU;;;/g, "").replace(/THARINDU;;;/gi, ""); // DONT REMOVE BLACK;;; so you can connect BLACK md session id too
...
await updateStatus()
 ...
 Void.ev.on('group-participants.update', async(anu) => {
        ...
        Void.ev.on('connection.update', async(update) => {
                ...
                if (connection === 'open') {
                    console.log("✅ Tharindu Login Successful!");
                    console.log("⬇️  Installing Tharindu External Plugins...");
                    ...
                    for (let i = 0; i < check.length; i++) {
                     ...
                     console.log("😁  External Plugins Installed of Tharindu!");fs.readdirSync(__dirname + "/../commands").forEach((plugin) => {
